...
public class ModifyPlanService : ServiceBase<ModifyPlanService, ProductDbContext, PlanDto, JResults<bool>>, IModifyPlanService
{
    /// <summary>
    /// ctor
    /// </summary>
    ...
    public override async Task<JResults<bool>> HandleAsync(PlanDto request, CancellationToken cancellationToken)
    {
        var exists = await this.DbContext.ProductPlans.FirstOrDefaultAsync(m => m.Id == request.Id, cancellationToken: cancellationToken);
        if(exists.xIsEmpty()) return await JResults<bool>.FailAsync("No product plan exists");
        ...
        if(approvalLines.xIsEmpty()) return await JResults<bool>.FailAsync("No approval lines exist");
        ...
        return await JResults<bool>.SuccessAsync(true);
    }
}