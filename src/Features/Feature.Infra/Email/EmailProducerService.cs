...
public class EmailProducerService : ServiceBase<EmailProducerService, EmailRequest, Results<bool>>, ISendEmailService
{
    ...
    public override async Task<Results<bool>> HandleAsync(EmailRequest request, CancellationToken cancellationToken)
    {
        var result = await _producer.ProduceAsync(TopicConstant.EmailTopic, new Message<Null, string> { Value = request.xSerialize()}, cancellationToken);
