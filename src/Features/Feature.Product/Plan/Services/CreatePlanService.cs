...
public class CreatePlanService : ServiceBase<CreatePlanService, ProductDbContext, PlanDto, Results<long>>, ICreatePlanService
{
    /// <summary>
    /// ctor
    /// </summary>
    ...
    public override async Task<Results<long>> HandleAsync(PlanDto dto, CancellationToken ct)
    {
        var exists = await this.DbContext.ProductPlans.FirstOrDefaultAsync(m => m.Id == dto.Id, ct);
        if(exists.xIsEmpty()) return await Results<long>.FailAsync("Already product plan exists");
        ...
        return await Results<long>.SuccessAsync(addItem.Id);
    }
}
